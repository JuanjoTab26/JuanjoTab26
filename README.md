# ¡Hola! Soy Juan José

### 🎓 Estudiante de Estadística | Universidad Nacional de Colombia

Actualmente soy estudiante de Estadística en la Universidad Nacional de Colombia. 
Desde niño he mostrado un gran interés y amor por los números, la lógica y la 
resolución de problemas, por lo que encontré en la Estadística una carrera que 
combina muchas de las áreas que más me apasionan.

---

## 📊 ¿Quién soy?

Soy un estudiante apasionado por los números y el conocimiento. 
Mi formación en Estadística me ha permitido acercarme al análisis de datos,
la probabilidad, el modelamiento y las herramientas computacionales.

Me interesa seguir desarrollando mis habilidades para formular preguntas,
analizar información y encontrar soluciones a problemas reales
mediante métodos matemáticos y estadísticos.

---

## 💰 Economía y Finanzas

Una de mis principales metas profesionales es especializarme en el área de 
**Economía y Finanzas**, utilizando la Estadística como una herramienta para 
analizar fenómenos económicos, financieros y de mercado.

Me interesa especialmente aprender sobre:

- 📈 Análisis de mercados
- 💰 Finanzas
- 📉 Modelamiento financiero
- 🎲 Probabilidad y riesgo
- 🔎 Análisis de datos económicos
- 🧮 Modelos estadísticos aplicados a las finanzas

Mi objetivo es construir un perfil que combine:

**📊 Estadística + 💻 Programación + 💰 Economía + 📈 Finanzas**

---

## 🧠 ¿Qué me interesa?

- 📊 Análisis y modelamiento estadístico
- 🎲 Probabilidad
- 📈 Inferencia estadística
- 📉 Series de tiempo
- 💰 Economía y Finanzas
- 🔎 Ciencia de datos
- 💻 Programación
- 📊 Visualización de datos
- 📈 Análisis de información

---

## 🛠️ Tecnologías y herramientas

### 📊 Estadística y análisis de datos

![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)

Actualmente continuo fortaleciendo mis capacidades en la programación, 
orientándolas al análisis de los datos y la resolución de problemas. 

---

## 🌎 Idiomas

**Español** — Lengua materna

**Inglés** — Segundo idioma

El inglés es una herramienta importante para mi formación académica y profesional,
especialmente para acceder a documentación, investigaciones y recursos relacionados con
Estadística, Economía, Finanzas y Ciencia de Datos.

---

## 🎓 Formación

**Estadística**  
Universidad Nacional de Colombia

Mi formación incluye áreas como:

- 🧮 Matemáticas
- 🎲 Probabilidad
- 📊 Estadística
- 💻 Programación
- 📈 Análisis de datos
- 📉 Modelamiento estadístico

---

## 🚀 Objetivos profesionales

Quiero construir progresivamente un perfil profesional que combine el 
pensamiento estadístico, las matemáticas y la programación con aplicaciones 
en Economía y Finanzas.

Me interesa desarrollar las capacidades necesarias para analizar datos, 
construir modelos estadísticos y utilizar herramientas cuantitativas para 
comprender problemas económicos y financieros.

---

## 📂 Proyectos

En este perfil iré compartiendo proyectos académicos y personales relacionados 
con:

📊 Análisis de datos  
🐍 Python  
📉 R  
💰 Economía y Finanzas      

### 💡 Los datos cuentan historias; la estadística nos ayuda a entenderlas.
